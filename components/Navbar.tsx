...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-200 border-b ${scrolled
        ? 'bg-neutral-950/95 border-neutral-800 py-3'
        : 'bg-neutral-950/0 border-transparent py-5'
      }`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-end md:justify-center items-center relative">
          ...
          <div className="flex gap-8 text-sm font-medium text-neutral-300">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className={`transition-colors relative py-2 ${activeSection === link.id ? 'text-violet-400' : 'hover:text-violet-400'
                  }`}
              >
                {link.name}
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-neutral-950 border-b border-neutral-800 py-6 px-6 flex flex-col gap-4 shadow-xl">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className={`text-lg font-medium transition-colors ${activeSection === link.id ? 'text-violet-400' : 'text-neutral-300 hover:text-white'
                }`}
